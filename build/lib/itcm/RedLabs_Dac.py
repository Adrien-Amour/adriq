...
from itcm.Count_Managers import sine_wave, MicromotionWindow
...
class TrapControlFrame(tk.Frame):
    def __init__(self, master=None, count_manager=None, default_h=0, default_v=0, default_trap_depth=0):
    ...
    def open_micromotion_window(self):
        ...
        self.micromotion_window = MicromotionWindow(self, self.count_manager)
        ...
        for i, h in enumerate(h_values):
            for j, v in enumerate(v_values):
                ...
                while total_counts < threshold and retries < 3:
                    try:
                        popt, hist, bin_edges = self.count_manager.RF_correlation(no_runs, rate, v_points)
...
class LoadControlPanel(tk.Frame):
    def __init__(self, parent, Count_Manager, Threshold, Timeout, *args, **kwargs):
    ...
    def load_function(self):
        ...
        # Ensure counting is true
        if self.Count_Manager and not self.Count_Manager.counting:
            self.Count_Manager.counting = True
        ...
        try:
            while self.loading:
                ...
                # Check if there are enough counts
                if self.Count_Manager and isinstance(self.Count_Manager.counts, list):
                    count_len = len(self.Count_Manager.counts)
                    if count_len > 0:
                        # Determine the number of counts to average
                        num_counts = min(count_len, 5)
                        recent_counts = self.Count_Manager.counts[-num_counts:]

                        # Flatten the list if it's a list of lists with single elements
                        flattened_counts = [item[0] for item in recent_counts if isinstance(item, (list, tuple)) and len(item) == 1 and isinstance(item[0], (int, float))]
                ...
                # Sleep for a duration based on the Count_Manager's rate
                time.sleep(1 / self.Count_Manager.rate if self.Count_Manager else 1)
