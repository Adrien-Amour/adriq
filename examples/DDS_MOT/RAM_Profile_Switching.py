...
def test_ram():
    ...
    # Ask for mode
    mode = input("Enter mode (slave (0)/master (1)/standalone (2)): ").strip().lower()
    ...
    ram_profile_setting(port, board, 1,
    PLL_Multiplier=40, Amplitude_Ramp_Rate_Divider=1, Start_Address=1, End_Address=1000, No_Dwell_High=True, Zero_Crossing=False, Profile_Mode='Continuous Recirculate')
    ...
    pulse_str = ''.join(pulse)
    pulse_out("COM5", pulse_str)
    Pulses = ['0100000000000000', pulse_str]
    Pulse_Lengths = [8, 4]
    write_pulse_sequencer("COM5", Pulses, Pulse_Lengths, Continuous=True)
    control_pulse_sequencer("COM5", 'Start')  # always stop pulse sequencer before a write operation
    time.sleep(20)
    control_pulse_sequencer("COM5", 'Start')  # always stop pulse sequencer before a write operation
