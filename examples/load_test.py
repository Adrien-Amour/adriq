...
detunings = np.arange(44,44.2,0.2)  # Detunings from -5 to 5 in steps of 0.5
efficiencies = []
...
for detuning in detunings:
    load_single_photon_experiment(detuning, 1, 1)

    exp_runner.clear_channels()
    exp_runner.start_experiment(N=5)
    
    total_counts = exp_runner.get_counts_in_window("signal-sp", lower_cutoff=21, upper_cutoff=25)
    #exp_runner.plot_time_diffs_histogram("signal-f",upper_cutoff=28)
    #exp_runner.plot_time_diffs_histogram("signal-sp",upper_cutoff=28)
    #exp_runner.plot_time_diffs_histogram("signal-sp",lower_cutoff=21, upper_cutoff=25)

    print("Total Counts:", total_counts)
    print("Current Width:", detuning)
    # Calculate efficiency for channel 4
    counts_channel_4 = total_counts.get('single_photon_chan4', 0)
    efficiency = (counts_channel_4 / 500000) * 100
...
plt.title('Efficiency vs. Width')
plt.xlabel(fr'Width ($\mu$s)')
